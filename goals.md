...
- [2026-06-13] M5: PS9 smoke. Część 1 (render, 0 kredytów) PASS — zakładka renderuje na Symfony route PS9, CSS/JS ładują się, odczyt Configuration OK, routing+proxy AJAX OK (getJob fejk → 404 w JSON). UI zgodne z decyzjami (bez dropdownu Model AI). Część 2 (generacja) złapała realny bug PS8↔9: `_PS_PROD_IMG_DIR_` niezdefiniowany w kontekście AdminQameraAjaxController na PS9 → 500 przy registerImage. Fix: helper `productImageDir()` z fallbackiem `_PS_IMG_DIR_.'p/'` / `_PS_ROOT_DIR_.'/img/p/'`. Po fixie Flow A pełny na PS9 (register→packshot→accept→reload z odtworzeniem stanu z API→generateSession). Sesja: backend `generation_failed` (timeout modelu seedream, nie moduł). Env PS9: świeży kontener wstaje z var/ niezapisywalnym (500) — wymaga `chown www-data var` (do README). (§8 prd.md, DoD M5)
...
- [2026-08-26] Weryfikacja po przerwie + grunt pod Addons: pytanie o wyciek z `GET /products/{ref}` ZAMKNIĘTE — wyniki są zawężone (`installation_id` + `product_id` w `listByProduct`, klucz API związany z instalacją; empirycznie 4 produkty, 0 naruszeń, `tools/probe-ref.php` przepisany na narzędzie orzekające z kodem wyjścia). `GET /images` nie istnieje (405), `GET /packshots` zawęża po `product_ref` — moduł nie woła żadnego z nich; jedyny odczyt ponad produkt to `GET /jobs`, filtr `product_ref` zacieśniony do fail-closed. Core Flow przeszedł end-to-end z karty produktu na PS 9.1.4 (produkt 1 / `ps-1`): upload+register → packshot job `c392d0fe` → polling → accept → sesja `256229d4` (pierwsza próba padła po stronie platformy, automatyczne ponowienie OK) → publikacja `id_image=28`. Złapany realny bug: `QameraApiClient::request()` brał poprawny `Job` z niepustym polem `error` za kopertę błędu, więc każde nieudane / `retry_pending` zadanie kończyło się „nieoczekiwanym błędem serwera" i zatrzymanym pollerem — naprawione (`isErrorEnvelope()`: 4xx/5xx albo `error` jedynym kluczem). Kształty żądań i odpowiedzi (13 wywołań) zwalidowane strukturalnie wobec `plugin-v1.yaml` — 0 odstępstw. Do Addons brakuje: decyzji o licencji (blokuje nagłówki w 16 plikach PHP), `logo.png`, `module_key`, `docs/` w module, angielskich tekstów źródłowych. Raport: `docs/weryfikacja-2026-08.md` (§3, §8 prd.md).
- [2026-08-26] Grunt pod Addons: licencja **AFL-3.0** (decyzja Pawła — ta sama, którą noszą moduły rdzenia PrestaShopa; permisywna, nie blokuje sprzedaży, a i tak nic nie chroni oprócz klucza API). Nagłówek „NOTICE OF LICENSE" w 16/16 plików PHP + `qameraai/LICENSE.md` (kopia tekstu, który PrestaShop dystrybuuje z `ps_themecusto`, nie przepisywana). Metadane na angielski (opis modułu, `config.xml`, `qameraai/README.md`), klucz tłumaczenia opisu przeliczony w `pl.php`/`en.php` — napisy w panelu zostają po polsku (decyzja: „tylko metadane"). Dopisane `9:16` do proporcji sesji (API to umiało, panel nie oferował). Pakiet przebudowany: 23 pliki z `LICENSE.md`, wyłącznie `qameraai/`. Po zmianie: `php -l` czyste na wszystkich plikach, zakładka renderuje się na PS9. Do zgłoszenia zostają `logo.png`, `module_key` (wartość z konta Addons) i `docs/` w module; walidator PrestaShopa nieuruchomiony — to wysyłka paczki na zewnątrz (§8 prd.md).
- [2026-08-26] Walidator PrestaShopa **nieosiągalny**: `validator.prestashop.com` nie ma już anonimowego wgrywania — `/` i `/module` przekierowują na `/auth/login` („Use your existing seller account"), `/validator`, `/upload`, `/search` dają 404, publiczne zostają tylko dokumentacja, changelog i generator, żadne bez pola na plik. Konto sprzedawcy poza zakresem zlecenia, więc walidator nieuruchomiony — to pozycja dla człowieka. Zamiast tego przepuszczony **standard kodu, który PrestaShop publikuje** (`prestashop/php-dev-tools` v5, `php-cs-fixer` z ich `PrestaShop\CodingStandards\CsFixer\Config`): 14/16 plików miało odstępstwa (pusta linia po `<?php`, wolnostojący komentarz jako `/**` zamiast `/*`, `array()` zamiast `[]`, `const` bez `public`, porządek znaczników docbloków) — poprawione ich narzędziem, drugie przejście 0/16. Po sweepie zweryfikowane: `php -l` czyste na **PHP 7.4** (pierwszy raz w tej sesji sprawdzona dolna granica zgodności) i 8.1, nagłówek licencyjny na wszystkich 16 plikach, zakładka renderuje się na PS9. PHPStan z ich rozszerzeniem dla modułów nie wystartował ani na 8.2.7, ani na 9.1.4 (`Class "LinkCore" not found`) — nieuruchomiony (§8 prd.md).
- [2026-08-26] Raport walidatora PrestaShopa (Paweł wgrał paczkę ze swojego konta — anonimowe wgrywanie już nie istnieje) **domknięty w całości**. Cztery grupy uwag: brak `.htaccess` w katalogu modułu (dołożony ten, którego PrestaShop używa we własnych modułach; sprawdzone: CSS/JS 200, PHP i listing 403), pusta linia przed nagłówkiem licencyjnym w 11 plikach (usunięta we wszystkich 16), brak nagłówka w plikach nie-PHP (dopisany do `qamera-product.js`, `product-tab.tpl`, `_packshot.tpl` i — profilaktycznie — `qamera-admin.css`), HTML sklejany w PHP (panel konta przeniesiony do `views/templates/admin/account-status.tpl`, escaping przez filtr Smarty, klucze tłumaczeń przeniesione do domeny szablonu). Siedem uwag z analizy statycznej też zamkniętych: `@return never` na `json()`/`apiError()` (fałszywe alarmy o niezdefiniowanych zmiennych — `json()` kończy się `exit`), martwe `$sha === ''`, `$tab->active` na `bool`, przepisane `uninstallDb()`, i jedna realna rozjazda — docblock `buildProductView()` nie wymieniał zwracanych `registered`/`truncated`. **Konflikt narzędzi PrestaShopa**: ich formater wstawia pustą linię po `<?php`, ich walidator za to odrzuca — repo ma teraz `.php-cs-fixer.dist.php` z ich zestawem reguł minus `blank_line_after_opening_tag`. Po poprawkach sprawdzone na żywo: strona ustawień renderuje panel konta z prawdziwymi danymi, zakładka produktu działa, składnia czysta na 7.4 i 8.1, paczka 26 plików. Zostają `logo.png`, `module_key`, `docs/` w module — żadnego walidator nie zgłosił (§8 prd.md).
- [2026-08-26] Walidator PrestaShopa po poprawkach: **„Validation passed"** (przebieg Pawła na jego koncie, nie widziany przeze mnie — drugiego raportu nie mam w repo). To zamyka automatyczną część walidacji technicznej; przegląd ręczny PrestaShopa to osobny etap. Dopisana dokumentacja w `qameraai/docs/` — przewodnik dla sklepikarza (co robi moduł, wymagania, instalacja, sześć kroków generacji, kredyty, tabela komunikatów błędów, granice wersji, dane i prywatność, wsparcie); źródło jako HTML w repo, PDF (format zalecany przez listę kontrolną) generowany przez `tools/build-docs.js`. To pozycja z listy kontrolnej, której walidator nie sprawdza. Paczka 29 plików. Zostają `logo.png` i `module_key` — oba po stronie konta sprzedawcy, żadnego walidator nie zgłasza (§8 prd.md).
- [2026-08-26] `logo.png` dołożone bez pracy graficznej: kanoniczny znak Qamery (przysłona-„Q", grafit `#252b30` + teal `#83babc`) leży w `saas-platform` jako ikona aplikacji 512×512 z alfą — przeskalowany do 140×140, rozmiaru używanego przez większość modułów rdzenia PrestaShopa, z zachowaniem przezroczystości. Sklep go serwuje, paczka ma 30 plików. Obrazy do samego listingu na Addons (okładka, zrzuty ekranu) wgrywa się w formularzu, nie w archiwum — poza zakresem. Zostaje `module_key` (wartość z konta sprzedawcy) (§8 prd.md).
- [2026-08-26] Adres `qamera.ai` usunięty z dokumentacji i tekstów interfejsu (decyzja Pawła). Powód: regulamin Addons zabrania podawania własnego adresu strony i e-maila **zarówno na stronie produktu, jak i w dokumentacji** — sprawdza to recenzja marketingowa (do 10 dni roboczych na rundę). Zostaje sama nazwa „Qamera AI"; w kodzie zostaje `DEFAULT_API_BASE`, bo to endpoint, nie odsyłacz. Zmienione: opis pola „Adres API" (klucz tłumaczenia przeliczony w `pl.php`/`en.php`), `qameraai/README.md` ×3, przewodnik w `qameraai/docs/` (HTML + przegenerowany PDF). **Uwaga: to zderza się z ustaleniem z daily 25.08 o „CTA kierującym na kontakt z nami" — kanał akwizycji przez wtyczkę opiera się teraz na samej nazwie, nie na odsyłaczu. Dotyczy tak samo WooCommerce.** Po zmianie: składnia czysta na 7.4, strona ustawień renderuje panel konta, paczka 30 plików (§8 prd.md).
